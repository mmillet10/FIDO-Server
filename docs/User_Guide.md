...
Policy Attribute(s) | Accepted Values &nbsp;&nbsp;&nbsp;[...] = Multiple Choice, Bold = Default  |  Usage Notes
  :---  |  :---  |  :--
| "cryptography":  |  |  Governs behavior of attestation, Elliptic Curves (EC), and signatures. EC tends to be faster and smaller than RSA, but is more complicated to implement. 
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"attestation_formats":  |  ["fido-u2f, "packed", "tpm", "android-key", "android-safetynet", "none"]<br><br>**Supported Attestation Types by Format**<br>"fido-u2f": _basic, attca_<br>"packed": _basic, self, attca, ecdaa_<br>"tpm": _attca, ecdaa_<br>"android-key": _basic_<br>"android-safetynet": _basic_<br>"none": _none_ <br>|  Choices for "attestation_types" (below) depend on the format chosen here. RP applications and servers determine acceptable attestation types by these policy parameters.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"displayName":  |  ["required", **"preferred"**]<br><br>"required": The RP web application will fail without the displayName.<br>"preferred": The RP web application prefers a displayName, but will not fail without one.  |  A label for the Authenticator.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"requireResidentKey":  |  [true, **false**] (**can be both&dagger;**)  |  If true, the Authenticator MUST create a client-side-resident public key credential source when creating a public key credential. If both true and false, failing to create a credential source will not prevent it from continuing.
...
"counter": |  |  A suggested feature allowing an RP web application to possibly detect that an Authenticator has been copied.
